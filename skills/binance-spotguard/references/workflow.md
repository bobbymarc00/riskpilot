...
## Relaxed PAPER intents

For trusted direct Telegram messages that are not `/spot` commands, run the exact text through:

```text
riskpilot --json paper-intent --text ORIGINAL_MESSAGE --sender-id ACTUAL_SENDER_ID --chat-id ACTUAL_CHAT_ID --notify
```

Pass the original message as one argv item and use only trusted inbound sender/chat metadata. For `buy BTC 10`, `buy 10 BTC`, and `buy BTC 10 usd`, run the shown `paper-intent` command immediately: the sole positive numeric value is always the maximum quote amount in USDT and never a base-asset quantity. Do not ask a preliminary USDT-vs-BTC question. Each form creates only a PAPER proposal; its `APPROVE PAPER` button is the required execution confirmation. `risk.default_order_size_usdt` is a default order size, not a maximum and never an implicit amount for incomplete buy text. Missing/multiple/non-positive/malformed amounts or unsupported symbols must fail or request clarification without a proposal. Text containing explicit `live`, `real`, or their localized equivalents buy intent is not normalized to PAPER and must direct the owner to the exact protected `/spot live-buy` route. This deterministic normalizer also supports English and Indonesian imperative forms for buying, closing (including explicit percentages), positions, and balance. Questions, quotations/forwards, conditionals, and generic natural-language approval fail closed. A percentage close is calculated over every open tranche of the symbol and still requires approval. Dust-producing partial closes fail before proposal creation. For a combined close-and-buy request it may create only the close proposal and must defer the buy; an explicit reject-pending plus buy performs only the rejection and asks for a new buy message. Never infer LIVE. Legacy `sg:` callbacks remain parseable only for compatibility; OpenClaw 2026.8.1 wraps generic presentation callbacks as unclaimed `tgcb1` data, so do not claim that a callback button reaches RiskPilot. Localized position typos are normalized using the input vocabulary in the locale catalogs. Invalid persisted brackets are reported as requiring local repair and must never be auto-closed or rewritten by Telegram.
