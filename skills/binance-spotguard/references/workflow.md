...
fallback prefix is `/spot`; native `/binance_spotguard` is a claimed `tgcmd:`
transport for structured controls below, including LIVE controls. Match native
LIVE controls before every conversational or generic-intent route.
- Native `/binance_spotguard review CANDIDATE_ID`: require the trusted owner/chat context and run `riskpilot --json agent-os review --candidate CANDIDATE_ID --notify`. This is a review action only. It verifies the candidate through the existing Binance Agent OS path and may create a proposal according to the existing RiskPilot policy; it never directly submits an order.
...
- Exact `/spot radar`: require the trusted owner and chat, then run `riskpilot --json radar`. Display the returned `presentation.text` verbatim. This reads only the latest local Smart Scanner snapshot, makes no Binance request, and never creates a candidate, proposal, or order.
...
- Exact `/spot live balance`: require trusted owner and chat metadata, then run `riskpilot --json live balance`. Read-only; render only returned `presentation.text`.
- Exact `/spot live positions`: require trusted owner and chat metadata, then run `riskpilot --json live positions`. Read-only; it includes balances and open TP/SL orders; render only returned `presentation.text`.
...
- Exact `/spot live-close-all SYMBOL`: require the owner and run `riskpilot --json live-exit-percent --symbol SYMBOL --percentage 100 --notify`. It requires exactly one active two-leg SELL OCO, then creates one dormant LIVE protected exit proposal. After approval it cancels that OCO and sells the protected quantity. It never sells before the native APPROVE LIVE button is pressed.
- Exact percentage `/spot live-exit-percent SYMBOL PERCENT`: require the owner and run `riskpilot --json live-exit-percent --symbol SYMBOL --percentage PERCENT --notify`. Percent must be >0 and <=100. The proposal displays the exchange-rounded sell amount and remaining amount. Approval performs cancel → sell → re-arm, with no automatic retry after an ambiguous result.
- Exact `/spot live-restore-protection SYMBOL`: require the owner and run `riskpilot --json live-restore-protection --symbol SYMBOL --notify`. It creates a dormant LIVE SELL OCO proposal using the most recent approved/reconciled bracket for that symbol and the current free balance. It must not create an order before APPROVE LIVE.
- Exact `/spot live-cancel-protection SYMBOL`: require the owner and run `riskpilot --json live-cancel-protection --symbol SYMBOL --notify`. It may cancel exactly one active two-leg Spot SELL OCO list for the symbol, after APPROVE LIVE only. It does not sell the asset. After cancellation is confirmed, use `/spot live-close-all SYMBOL` to create a separate sell proposal.
- Native `/binance_spotguard live-approve PROPOSAL_ID` and `live-reject PROPOSAL_ID` are the sole LIVE confirmation controls. Match these exact structured commands first and route them with actual trusted inbound sender/chat metadata to `riskpilot --json live-approve PROPOSAL_ID --sender-id ACTUAL_SENDER_ID --chat-id ACTUAL_CHAT_ID` or `live-reject`. If either metadata value is missing, reject clearly and fail closed: never substitute configured owner/chat IDs, invoke an LLM, recreate a proposal, or reinterpret it as trade intent. The short native command fits Telegram's callback limit; the approval token remains local and is re-derived from the immutable proposal.
- Exact `/spot live status`: require the owner and run `riskpilot --json live status`.
- The five trusted natural-language LIVE wizard intents are deterministic Telegram routes, not generic LLM requests: `cek live readiness`; `aktifkan mode live`; exact `READY TO LIVE TRADE`; `nonaktifkan mode live`; and exact `DISABLE LIVE TRADING`. Route each through `riskpilot --json telegram-live --text ORIGINAL_MESSAGE --sender-id ACTUAL_SENDER_ID --chat-id ACTUAL_CHAT_ID` before any LLM response. The activation phrase is valid only for its active, one-time, sender/chat-bound challenge and grants a seven-day authorization only after full preflight. It does not approve or submit an order; every LIVE trade still needs its existing proposal and human approval. Disable revokes/disarms new entry only and preserves protective exits. Other `/spot live pause`, enable, arm, disable, disarm, and scheduled-mode commands remain local interactive VPS controls.
...
- `/binance scan`: the scheduled scanner is temporarily disabled during Binance public-REST rate-limit/IP-ban optimization. Report that no background or manual broad scan should run until Operations work is complete; do not invoke `scan --notify` as a workaround. Small explicit analysis requests remain read-only and should be used sparingly.
...
Reject `/binance approve ...`, plain `APPROVE`, and natural-language approval requests. Natural-language buy must always run `trade-intent`, never `paper-intent`: it creates a LIVE proposal by default, while only an explicit `paper buy ...` creates a PAPER proposal. Natural-language `sell all SYMBOL` and `sell SYMBOL 30%` follow `trade-intent` and create LIVE protected exit proposals by default; only an explicit `paper sell ...` stays PAPER. Only a proposal button can authorize a live order; never offer or accept a typed live approval fallback.

For trusted direct Telegram only, clear natural read-only requests route before the LLM: `check my live balance`, `live balance`, and `check balance` run `riskpilot --json live balance`; `open positions`, `current position`, `check my balance and open position`, and `check all balance and open position` run `riskpilot --json live positions`. The positions result is the combined balance/positions response. Ambiguous text remains unhandled; this classifier never recognizes `approve`, `yes`, `gas`, or any trade request. Return only `presentation.text`, never raw CLI JSON or internal orchestration syntax.

For every `/spot live-buy SYMBOL AMOUNT`, invoke the project executable at `${OPENCLAW_WORKSPACE:-$HOME/.openclaw/workspace}/tools/spotguard-agent-os/riskpilot` with `--json live-buy --symbol SYMBOL --quote-amount AMOUNT --notify`; do not use an older global `spotguard` executable or a paper command. For `/spot live-close-all SYMBOL`, invoke the same executable with `--json live-exit-percent --symbol SYMBOL --percentage 100 --notify`. For `/spot live-exit-percent SYMBOL PERCENT`, use `--json live-exit-percent --symbol SYMBOL --percentage PERCENT --notify`.
