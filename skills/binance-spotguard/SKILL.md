...
Handle the registered `/spot` routes and native `/binance_spotguard` button routes, the explicitly documented relaxed PAPER intents, and Telegram callback values beginning with `sg:`. `/risk` is not a registered OpenClaw command in this installation and must not be displayed as a fallback. Use the bundled RiskPilot CLI as the source of truth for candidates, risk limits, proposal status, approval tokens, and execution leases. Native controls and clear trusted Telegram read-only intents are dispatched before any LLM turn; never expose orchestration syntax, tool envelopes, or JSON to Telegram users.
...
- For trusted direct Telegram only, route clear live read-only intent deterministically: `check my live balance`, `live balance`, or `check balance` runs `riskpilot --json live balance`; `open positions`, `current position`, `check my balance and open position`, and `check all balance and open position` runs `riskpilot --json live positions` (which already includes balances and protected TP/SL orders). Do not let an LLM claim this feature is unsupported. Missing trusted sender or chat metadata fails closed, with no configured-ID substitution.
- Display returned LIVE approval `presentation.text` verbatim. If immutable `remaining_quantity == 0`, label the protected exit as a **full exit**, never a partial exit; `remaining_quantity > 0` remains a partial exit. Never substitute PAPER approval/rejection wording for native LIVE actions.
